...
- **🏃 Course & Marche (GPS + OpenStreetMap)** : mode cardio sans caméra. Carte **Leaflet auto-hébergée** (open-source), suivi GPS temps réel (distance, allure, durée, tracé), détection course/marche automatique. Récompense **1 tick/100 m** re-validé côté serveur (min 20 s/100 m = anti-véhicule). **Cardio Bluetooth** optionnel (`Web Bluetooth`, ceintures/bracelets BLE standard `0x180D`, Android Chrome). **Import GPX** (bouton 📁) pour visualiser une trace exportée d'Apple Santé / Huawei Health / Strava — *visualisation + résumé uniquement, sans sats (fichier non vérifiable)*.
