...
- **🧠 Coach IA (optionnel — LLM Kimi/Moonshot)** : si `KIMI_API_KEY` est configurée, le récap inclut une **analyse personnalisée** rédigée par le LLM (débrief + focus, **streamée en direct SSE**) et un bouton **Plan de la semaine** (tenant compte du ratio de charge **ACWR** 7 j/28 j). **Confidentialité** : seules des métriques bornées et des identifiants d'exercice en liste blanche partent vers l'API — jamais la vidéo, jamais de texte libre, jamais de pseudo (anti-injection par construction). Coût borné : connexion requise + `COACH_DAILY_CAP` appels/jour/compte (défaut 15). **Pack v1** : 🎭 4 personas (Coach/Sergent/Zen/Nerd), ⚡ **séances générées directement jouables** (le LLM écrit le programme en JSON validé côté serveur, l'app l'exécute en mode guidé avec enchaînement d'exercices), 📸 **analyse de posture par photo** (modèle vision — le **visage est pixelisé sur l'appareil** grâce aux landmarks, jamais envoyé), 🗣️ cue vocal de **fatigue** (VBT ci-dessous).
- **📉 VBT (Velocity-Based Training)** : vitesse concentrique de chaque rep mesurée sur les keypoints (hanches, poignets pour la boxe) ; perte >20 % vs ta meilleure rep de la séance → cue vocal « fatigue » + métrique envoyée au coach. La méthode des pros, sur téléphone.
- **👻 Mode fantôme** : ta meilleure séance sur chaque exercice est enregistrée (offsets des reps, 100 % local) et rejouée en direct — le chip `👻 record · toi` te dit si tu tiens le rythme, et battre ton fantôme déclenche l'euphorie générale.
- **🤸 3 nouveaux exercices** : **burpees** (machine à états squat→planche→squat), **abdos sit-ups** (angle du buste au sol), **mountain climbers** (genoux poitrine en alternance) — comptage caméra, objectifs hebdo et scoring serveur inclus. Seuils initiaux calibrables via `?cal=1`.
...
| `KIMI_API_KEY` | **Encrypt** 🔒 | clé API Moonshot/Kimi — active le **coach IA** (débrief + plan hebdo + séances + posture) |
| `KIMI_MODEL` | Plaintext | défaut **`kimi-k2.6`** (id exact vérifié via `GET /v1/models`) |
| `KIMI_VISION_MODEL` | Plaintext | défaut **`kimi-latest`** — modèle multimodal pour l'analyse de posture photo |
